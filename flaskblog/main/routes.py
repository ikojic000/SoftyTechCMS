...
def home():
    ...
    if request.method == "POST":
        if form.validate_on_submit:
            ...
            reviews = (
                Post.query.order_by(Post.date_posted.desc())
                .filter_by(category="Reviews")
                .first()
            )
            commentary = (
                Post.query.order_by(Post.date_posted.desc())
                .filter_by(category="Commentary")
                .first()
            )
            print("applying")
            print(posts)
        ...
        posts = (
            Post.query.filter_by(isPublished=True)
...
def news():
    page = request.args.get("page", 1, type=int)
    posts = (
        Post.query.filter_by(category="News")
        .order_by(Post.date_posted.desc())
    ...
    return render_template(
        "news.html", title="News", posts=posts, reviews=reviews, commentary=commentary
    )
...
def reviews():
    page = request.args.get("page", 1, type=int)
    posts = (
        Post.query.filter_by(category="Reviews")
        .order_by(Post.date_posted.desc())
        .paginate(page=page, per_page=2)
    )
    news = (
        Post.query.order_by(Post.date_posted.desc()).filter_by(category="News").first()
    )
    commentary = (
        Post.query.order_by(Post.date_posted.desc())
        .filter_by(category="Commentary")
...
def commentary():
    page = request.args.get("page", 1, type=int)
    posts = (
        Post.query.filter_by(category="Commentary")
        .order_by(Post.date_posted.desc())
        .paginate(page=page, per_page=2)
    )
    news = (
    ...
    return render_template(
        "commentary.html", title="Commentary", posts=posts, news=news, reviews=reviews
    )
...
def post(slug):
    ...
    print("TEST - main.post -> " + request.referrer)
    ...
    headImg = url_for(
        "static", filename="upload/media/images/head_Images/" + post.headImg
    )
    form = CommentForm()
    if form.validate_on_submit():
        comment = Comment(
